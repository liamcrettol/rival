-- Friends are useful in direct H2H search, but they distort "best rival"
-- rankings when private matches or repeated group play are present. Keep the
-- underlying encounters and search behavior unchanged; only omit known
-- Bungie-friend pairs from the two dashboard leaderboards.
create table if not exists public.rivalry_exclusions (
  viewer_membership_id text not null,
  excluded_membership_id text not null,
  source text not null default 'bungie_friend',
  confirmed_at timestamptz not null default now(),
  primary key (viewer_membership_id, excluded_membership_id),
  check (viewer_membership_id <> excluded_membership_id)
);

alter table public.rivalry_exclusions enable row level security;
