...
export default function MatchHallOfFame() {
  ...
  if (performances.length === 0) return <div className="border border-bungie-border bg-bungie-dark/50 px-4 py-8 text-center"><Trophy className="mx-auto text-gray-600" size={24} /><p className="mt-3 text-sm font-semibold text-white">{degraded ? "Full match history still loading" : "No qualifying wins yet"}</p><p className="mx-auto mt-1 max-w-md text-xs leading-relaxed text-gray-500">{degraded ? "Please check back later while the shared match stats finish loading." : "3v3 wins against an opponent with a 1.5+ lifetime Trials K/D will appear here once the shared stats cache has been populated."}</p></div>;
  return <div className="space-y-3">{degraded && <p className="border border-bungie-border/70 bg-bungie-dark/50 px-3 py-2 text-[11px] uppercase tracking-[0.14em] text-gray-500">Showing cached results, shared match stats are still updating.</p>}{performances.map((match) => <MatchCard key={match.instanceId} match={match} />)}</div>;
}
