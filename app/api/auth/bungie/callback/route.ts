...
export async function GET(req: NextRequest) {
  ...
  // check (a cross-service call to Rerolled) - only genuinely new sign-ins
  // need to reserve a lifetime slot. A failed local lookup falls through to
  // the capacity check below, so no capacity-safety guarantee is weakened
  // for actual new signups; it just means an outage of the local lookup
  // behaves like it did before this fix.
